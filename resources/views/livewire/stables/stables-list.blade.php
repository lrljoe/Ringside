<div class="card">
    @include('livewire.stables.partials.header')
                ...
                <x-slot name="body">
                    @forelse ($stables as $stable)
                        <x-table.row :class="$loop->odd ? 'odd' : 'even'" wire:loading.class.delay="opacity-50" wire:key="row-{{ $stable->id }}">
                            <x-table.cell>
                                <x-form.inputs.checkbox wire:model="selected" value="{{ $stable->id }}" />
                            </x-table.cell>

                            <x-table.cell>
                                <a class="mb-1 text-gray-800 text-hover-primary" href="{{ route('stables.show', $stable) }}">{{ $stable->name }}</a>
                            </x-table.cell>
                            ...
                            <x-table.cell>
                                {{ $stable->created_at->toFormattedDateString() }}
                            </x-table.cell>
                        ...
                        <x-table.row>
                            <x-table.cell colspan="6">
                                <div class="flex items-center justify-center space-x-2">
                                    <span class="py-8 text-xl font-medium text-cool-gray-400">No stables found...</span>
                                </div>
                            </x-table.cell>
                        </x-table.row>
                    @endforelse
