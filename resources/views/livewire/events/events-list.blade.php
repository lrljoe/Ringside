                ...
                <x-slot name="body">
                    @forelse ($events as $event)
                        <x-table.row :class="$loop->odd ? 'odd' : 'even'" wire:loading.class.delay="opacity-50" wire:key="row-{{ $event->id }}">
                            <x-table.cell>
                                <x-input.checkbox wire:model="selected" value="{{ $event->id }}" />
                            </x-table.cell>

                            <x-table.cell>
                                <a class="mb-1 text-gray-800 text-hover-primary" href="{{ route('events.show', $event) }}">{{ $event->name }}</a>
                            </x-table.cell>

                            <x-table.cell>
                                <div class="badge badge-{{ $event->status->getBadgeColor() }}">
                                    {{ $event->status->label }}
                                </div>
                            </x-table.cell>
