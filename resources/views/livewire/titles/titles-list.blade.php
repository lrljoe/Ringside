<x-card>
    ...
    <x-card.body class="pt-0">
        <x-table.wrapper>
            <x-table>
                <x-table.head>
                    <x-table.heading class="w-10px pe-2 sorting_disabled"><x-titles.index.check-all/></x-table.heading>
                ...
                <x-table.body>
                    @forelse ($titles as $title)
                        <x-table.row :class="$loop->odd ? 'odd' : 'even'" wire:loading.class.delay="opacity-50" wire:key="row-{{ $title->id }}">
                            <x-table.cell><x-form.inputs.checkbox wire:model="selectedTitleIds" value="{{ $title->id }}" /></x-table.cell>
