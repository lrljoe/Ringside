<x-card>
    ...
    <x-card.body class="pt-0">
        <x-table.wrapper>
            <x-table>
                <x-table.head>
                    <x-table.heading class="w-10px pe-2 sorting_disabled"><x-venues.index.check-all/></x-table.heading>
                ...
                <x-table.body>
                    @forelse ($venues as $venue)
                        <x-table.row :class="$loop->odd ? 'odd' : 'even'" wire:loading.class.delay="opacity-50" wire:key="row-{{ $venue->id }}">
                            <x-table.cell><x-form.inputs.checkbox wire:model.live="selectedVenueIds" value="{{ $venue->id }}" /></x-table.cell>
