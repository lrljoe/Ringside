                        ...
                        <x-table.row :class="$loop->odd ? 'odd' : 'even'" wire:key="row-{{ $previousTitleChampionship->title_id }}">
                            ...
                            <x-table.cell>
                                @if ($previousTitleChampionship->title->championships->first()->currentChampion->is($this->tagTeam))
                                    {{  "First Champion" }}
                                @else
                                    <x-route-link :route="route('tag-teams.show', $previousTitleChampionship->title->championships->first()->currentChampion)" label="{{ $previousTitleChampionship->title->championships->first()->currentChampion->name }}"/>
                                @endif
                            </x-table.cell>
