...
class ViewWrestlersListTest extends TestCase
{
     ...
     * @dataProvider administrators
     */
    public function an_employment_is_created_for_the_wrestler_if_activated_at_is_filled_in_request($administrators)
    {
        $activatedAt = now()->toDateTimeString();

        $this->actAs($administrators);

        $this->storeRequest('wrestlers', $this->validParams(['activated_at' => $activatedAt]));

        tap(Wrestler::first(), function ($wrestler) use ($activatedAt) {
            $this->assertCount(1, $wrestler->employments);
            $this->assertEquals($activatedAt, $wrestler->employments->first()->started_at->toDateTimeString());
        });
    }
     ...
     * @dataProvider administrators
     */
    public function update_a_wrestler($administrators)
    {
        $this->actAs($administrators);
        $wrestler = WrestlerFactory::new()->create();
     ...
     * @dataProvider administrators
     */
    public function an_administrator_can_delete_a_wrestler($administrators)
    {
        $this->actAs($administrators);
        $wrestler = WrestlerFactory::new()->create();
